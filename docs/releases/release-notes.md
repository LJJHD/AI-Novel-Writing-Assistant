...
- OpenAI 兼容中转返回顶层错误信息时，连接测试会直接显示中转返回的原因，例如余额不足或模型未授权，不再显示 SDK 内部解析报错。
...
- 新增 macOS 桌面打包入口，源码环境中可以执行 `pnpm run dist:desktop:mac` 生成 Apple Silicon 桌面包，也可以执行 `pnpm run dist:desktop:mac:x64` 生成 Intel Mac 桌面包。
- 桌面图标资产补齐 macOS 所需的 `.icns`，打包后的应用会使用同一套品牌图标。
- 桌面包验证脚本可以按 Windows / macOS 分别检查打包后的资源目录、前端入口、本地服务入口和 Prisma 运行时，降低跨平台产物遗漏资源的风险。
