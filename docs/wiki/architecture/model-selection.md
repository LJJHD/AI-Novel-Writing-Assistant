...
- 自定义厂商连接测试报 `Cannot read properties of undefined (reading 'message')`：先确认连接测试是否走了手写 OpenAI-compatible chat probe，并查看响应结构摘要。可能原因包括 API URL 未指向 `/v1` 兼容根路径、响应体不是标准 Chat Completion、`choices` 为空、`choices[0].message` 缺失、顶层 `{ code, message }` 错误、账户余额不足或当前模型未授权；在没有原始响应或探针摘要前，不应直接断定是中转平台问题。
