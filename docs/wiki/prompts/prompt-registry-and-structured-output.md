...
- OpenAI 兼容中转如果返回空 choices 或空 message，可能在 SDK 层表现为 `Cannot read properties of undefined (reading 'message')`。结构化输出层必须把这类传输异常归一为可操作的模型端点提示，并让任务中心、自动导演阻塞原因、检查点摘要复用同一归一化结果；不要把 SDK 内部 TypeError 直接展示给用户。
