...
AIPORT 等 OpenAI 兼容中转的结构化任务报错更清楚：当模型端点返回空响应或当前模型未按聊天补全协议返回内容时，任务中心和自动导演会提示检查 OpenAI 兼容接口与授权文本模型，不再显示 SDK 内部的 `undefined.message` 报错。
...
- 连接测试、结构化探针和自动导演任务流对这类空响应错误保持一致提示，便于直接回到模型配置修正。
