...
AIPORT 等 OpenAI 兼容中转的结构化任务报错更清楚：当 SDK 没有拿到标准 OpenAI Chat Completion 消息时，任务中心和自动导演会提示检查 API URL、请求协议、聊天补全支持和模型授权，不再显示 SDK 内部的 `undefined.message` 报错。

- 小说主流程的阻塞原因、检查点摘要和失败摘要会统一显示可操作的模型端点提示。
- 已经失败的历史任务如果保存了同类结构化输出错误，重新打开任务详情时也会按同一规则展示。
- 任务中心、Creative Hub、Agent 工具、拆书和知识库索引等入口会复用同一套错误说明，减少不同页面展示不同错误文本的情况。
- OpenAI 兼容连接探针会在 200 但响应结构不标准时显示响应结构摘要，例如 `choices` 是否为空或 `choices[0].message` 是否缺失，便于判断是地址、协议、模型能力还是授权配置问题。
- 自动导演等真实结构化调用也会记录 OpenAI 兼容响应结构诊断；如果失败只剩传输错误，任务详情会保留底层原因，日志中可继续查看状态码、响应类型和响应结构摘要。
- 自动导演跟进详情里的检查点摘要也会按同一规则归一化，不再直接显示 `[STRUCTURED_OUTPUT:transport_error] Cannot read properties...` 这类 SDK 内部错误。
