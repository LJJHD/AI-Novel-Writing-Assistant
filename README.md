...
AIPORT 等 OpenAI 兼容中转的自动导演结构化调用更稳：如果中转在非流式请求里返回了标准 Chat Completion JSON，但把响应类型错标成事件流，系统会按非流式 JSON 正确处理，减少自动导演卡在模型传输错误上的情况。

- 普通聊天可用、结构化任务失败的模型连接更容易恢复，不再因为响应头不一致被 SDK 误判成流式解析错误。
- 真实流式响应会直接透传，不会因为诊断读取整段事件流而拖慢聊天、章节生成等需要边生成边显示的入口。
- 模型选择与排障说明补充了这类中转兼容问题，后续遇到 `message` 或 `map` 相关 SDK 内部错误时，可以更快定位到响应头与请求模式是否一致。
### 2026-06-05

自动导演连续执行更稳，章节生成上下文更聚焦：系统会减少无明确目标窗口的伏笔逾期误判，避免批量写章被重复账本错误打断；本书世界、章节任务、角色和冲突也会更稳定地进入后续章节上下文。
...
- 世界观从字段表单升级为世界骨架、本书世界和世界手册流程，自动导演可以在宏观规划后自动准备本书世界。
